...
| 호출형 감시 | 감시 시작/종료는 Android 루틴 또는 화면 버튼이 명시적으로 호출합니다. Bluetooth 연결 이벤트에서 시작하고 연결 해제 이벤트에서 종료하도록 연동할 수 있습니다. |
...
- 차량 목록, `vehicle_data`, `wake_up` 요청을 Worker 경유로 전달. 성공한 `vehicle_data`는 세션·VIN별 5분 동안 KV에서 반환해 Fleet API 호출을 제한
...
5. 등록 뒤에는 화면의 **감시 시작** 또는 Android Bluetooth 연결 루틴에서 `CameraMonitorService.start(context)`를 호출합니다. 연결 해제 루틴에서는 `CameraMonitorService.stop(context)`를 호출합니다.

감시 서비스는 자체 BLE 검색·재시작이나 Fleet D/R 상태로 시작하지 않습니다. 감시 중에는 지속 알림이 표시되며, 앱을 뒤로 보내거나 화면을 꺼도 동작합니다. 앱 강제 종료, 재부팅, 권한 해제, Bluetooth 해제 루틴 또는 수동 중지 시에는 멈춥니다.
